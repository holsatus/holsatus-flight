...
async fn main(thread_spawner: embassy_executor::Spawner) {
    ...
    // No override_imu_rot() here: HilImu feeds data directly in drone-body
    // frame (the host builds it, there is no physical chip mounting to
    // correct for), so we keep the imu_reader rotation at its Identity
    // default instead of applying BMI088_CHIP_TO_DRONE_ROT.
    ...
    // No physical imu_reader_task spawn in HIL -- hil_imu_task feeds
    // imu_reader::main_6dof from HilImu (injected over the link) instead
    // of the BMI088 driver.
    ...
    thread_spawner.spawn(hil_link_task(r.uart_log).unwrap());
...
async fn uart_writer_task(bt: BtLogResources, sd: SdmmcLogResources) -> ! {
    ...
    // USART1 (the wired FTDI line, normally the text log in free_test.rs)
    // is NOT opened here in the HIL binary -- hil_link_task owns it
    // exclusively for the binary sensor/attitude wire protocol. Text logs
    // go to the onboard BT module (UART8) and SD only. If BT init fails we
    // silently continue -- SD logs still work.
    ...
    if !sd_ok {
        ...
        loop {
            ...
            write_line(&mut bt_uart, msg.as_bytes()).await;
        }
    }
    ...
    loop {
        ...
        let msg = match select(ulog::CRITICAL_CHANNEL.receive(), ulog::CHANNEL.receive()).await {
            Either::First(m) => {
                write_line(&mut bt_uart, m.as_bytes()).await;
...
//
// hil_link_task owns USART1 (the same wire free_test.rs uses for text logs;
// uart_writer_task above deliberately does not open it in this binary) and
// speaks a small fixed-size binary protocol: it decodes inbound SensorFrames
// into Imu6DofData and pushes them into HIL_IMU_CHAN, and replies with the
// latest AHRS_ATTITUDE_Q as an AttitudeFrame. hil_imu_task feeds that channel
// into common::tasks::imu_reader::main_6dof via the HilImu adapter, exactly
...
async fn hil_link_task(r: UartLogResources) -> ! {
    ...
    let (mut tx, mut rx) = Uart::new(r.usart, r.rx, r.tx, r.dma_rx, r.dma, UartIrqs, cfg)
        .unwrap()
        .split();
