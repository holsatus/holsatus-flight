...
use crate::{
    errors::adapter::sequential_storage::SequentialError, params::registry::PARAM_REGISTRY,
    sync::procedure::Procedure,
};

use super::{
    ParamError,
    storage::{ParamStorage, WrappedValue},
};
...
pub async fn entry<F: NorFlash>(flash: F, range: Range<u32>) -> ! {
    StorageTask::new(flash, range).run().await
}

impl<F: NorFlash> StorageTask<F> {
    pub fn new(flash: F, range: Range<u32>) -> Self {
        Self {
            storage: ParamStorage::new(flash, NoCache::new(), range),
        }
    }
    ...
    pub async fn run_inner(&mut self) -> Result<(), Error> {
        ...
        // Ensure the requestee is made aware of any failure
    ...
    pub async fn handle_save_param(&mut self, ident: Ident) -> Result<(), Error> {
        let value = PARAM_REGISTRY.get_param(ident.as_raw()).await?;
    ...
    pub async fn handle_save_table(&mut self, name: &str) -> Result<(), Error> {
        let table = PARAM_REGISTRY
            .get_table(name)
            .ok_or(ParamError::NoMachingTable)?;
        let params = table.pure_read().await;
    ...
    pub async fn handle_load_table(&mut self, name: &str) -> Result<(), Error> {
        let table = PARAM_REGISTRY
            .get_table(name)
            .ok_or(ParamError::NoMachingTable)?;
        let mut params = table.pure_write().await;
