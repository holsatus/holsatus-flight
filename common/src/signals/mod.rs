...
macro_rules! multi_watch {
    ($name:ident, $type:ty, $count:ident, $subs:literal) => {
        pub static $name: [Watch<$type, $subs>; $count] = {
            const REPEAT: Watch<$type, $subs> = Watch::new();
            [REPEAT; $count]
        };
    };
}
...
macro_rules! multi_receiver {
    ($name:path, $count:ident) => {{
        let arr: [_; $count] = core::array::from_fn(|idx| unwrap!($name[idx].receiver()));
        arr
    }};
}
...
pub static RAW_IMU_DATA: Watch<Imu6DofData<f32>, 2> = Watch::new();
pub static RAW_MAG_DATA: Watch<Imu9DofData<f32>, 2> = Watch::new();

// Calibrated sensor data.
// NOTE: For redundant sensors we need an array of these!
pub static CAL_IMU_DATA: Watch<Imu6DofData<f32>, 3> = Watch::new();
pub static CAL_MAG_DATA: Watch<Imu9DofData<f32>, 2> = Watch::new();

/// Estimated vehicle attitude
pub static AHRS_ATTITUDE_Q: Watch<UnitQuaternion<f32>, 2> = Watch::new();
pub static AHRS_ATTITUDE: Watch<[f32; 3], 2> = Watch::new();

/// Raw data packet from GNSS module
pub static RAW_GNSS_DATA: Watch<GnssData, 2> = Watch::new();
pub static ESTIMATED_POS: Watch<[f32; 3], 2> = Watch::new();
pub static ESTIMATED_VEL: Watch<[f32; 3], 2> = Watch::new();

// Setpoint signals for the controller tasks
pub static TRUE_THROTTLE_SP: Watch<f32, 2> = Watch::new();
pub static TRUE_VELOCITY_SP: Watch<[f32; 3], 2> = Watch::new();
pub static TRUE_ANGLE_SP: Watch<[f32; 3], 2> = Watch::new();
pub static TRUE_RATE_SP: Watch<[f32; 3], 2> = Watch::new();
pub static SLEW_RATE_SP: Watch<[f32; 3], 2> = Watch::new();
pub static FF_PRED_GYR: Watch<[f32; 3], 2> = Watch::new();

// Setpoint outputs, to be routed by the signal_router task
pub static POS_TO_VEL_SP: Watch<[f32; 3], 2> = Watch::new();
pub static VEL_TO_ANGLE_SP: Watch<[f32; 3], 2> = Watch::new();
pub static ANGLE_TO_RATE_SP: Watch<[f32; 3], 2> = Watch::new();
pub static RC_AXES_SP: Watch<[f32; 3], 2> = Watch::new();

// Data received from an RC controller
pub static RC_CHANNELS_RAW: Watch<Option<[u16; 16]>, 2> = Watch::new();
pub static RC_ANALOG_RATE: Watch<RcAnalog, 2> = Watch::new();
pub static RC_ANALOG_UNIT: Watch<RcAnalog, 2> = Watch::new();
pub static RC_STATUS: Watch<RcStatus, 2> = Watch::new();

pub static RATE_PIPELINE_TIME: Watch<[Duration; 4], 2> = Watch::new();

//
pub static RATE_PID_TERMS: Watch<[PidTerms; 3], 2> = Watch::new();
pub static COMP_FUSE_GYR: Watch<[f32; 3], 2> = Watch::new();

pub static ANGLE_PID_TERMS: Watch<[PidTerms; 3], 2> = Watch::new();

// Control mode, defined by the commander task
pub static CONTROL_MODE: Watch<ControlMode, 2> = Watch::new();
...
// Commander signals
pub static CMD_ARM_MOTORS: Watch<(bool, bool), 3> = Watch::new();
pub static CMD_CALIBRATE: Watch<Calibrate, 2> = Watch::new();

/// The boot configuration is only read once at startup
pub static BOOT_CONFIG: OnceLock<BootConfig> = OnceLock::new();

// CONFIGURATION SIGNALS //
pub static CFG_RC_BINDINGS: Watch<RcBindings, 2> = Watch::new();
pub static CFG_MOTOR_DIRS: Watch<[bool; 4], 2> = Watch::new();
pub static CFG_VEHICLE_INFO: Watch<Vehicle, 2> = Watch::new();
pub static CFG_LOG_PRESET: Watch<LogPreset, 2> = Watch::new();

pub static CFG_RATE_LOOP_PIDS: Watch<RatePidCfg3D, 2> = Watch::new();
pub static CFG_CONTROL_FREQ: Watch<u16, 4> = Watch::new();
...
pub static IMU_MODES: Watch<[Mode; NUM_IMU], 2> = Watch::new();
