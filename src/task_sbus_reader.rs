...
pub async fn sbus_reader(
    ...
    loop {
        match with_timeout(config::definitions::SBUS_PARSE_TIMEOUT, uart_rx_sbus.read(&mut read_buffer)).await {
            Ok(Ok(())) => {
                ...
                } else if Instant::now().duration_since(prev_parse_time) > config::definitions::SBUS_PARSE_TIMEOUT {
