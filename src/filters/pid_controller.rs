...
impl<F: Float> LpFilter<F> {
    pub fn new(tau: F, ts: F) -> Self {
        LpFilter { signal: F::zero(), a: ts / (ts + tau), one_minus_a: tau / (ts + tau)}
    }
}

#[derive(Copy, Clone, Debug)]
pub struct PidConfig<F> {
    pub kp: F,
    pub ki: F,
    pub kd: F,
    pub ideal: bool,
...
impl<F: Float + SubAssign + AddAssign + PartialOrd> Pid<F>{
    ...
    pub fn with_config(config: PidConfig<F>, ts: F) -> Self {
        Pid {
            kp: config.kp,
            ki: if config.ideal { config.ki * config.kp } else { config.ki },
            kd: if config.ideal { config.kd * config.kp } else { config.kd },
            ts,
            integral: F::zero(),
            integral_en: true,
            prev_error: F::zero(),
            ...
            lp_filter: config.lp_filter.map(|tau| LpFilter::new(tau, ts)),
        }
    }
    ...
    pub fn set_lp_filter(mut self, tau: F) -> Self {
        ...
        self.lp_filter = Some(LpFilter { signal: F::zero(), a: self.ts / (self.ts + tau), one_minus_a: tau / (self.ts + tau)});
        self
    }
    ...
    fn apply_lp_filter(&mut self, input: &mut F) {
        if let Some(lp_filter) = self.lp_filter.as_mut() {
            lp_filter.signal = lp_filter.signal * lp_filter.one_minus_a + *input * lp_filter.a;
            *input = lp_filter.signal;
        } 
    }
